...
主体验必须同时成立于手机浏览器和桌面浏览器。两端共享信息架构，但**不能只做尺寸缩放**。

## 15.1 断点行为

```text
< 640px       Mobile：单列、底部导航、Bottom Sheet
640–1023px    Compact：仍以 Mobile 交互为主，适度增加留白/双列局部
>= 1024px     Desktop：左 Rail + 中央舞台 + 条件性右上下文 / Drawer
```

断点决定的是**布局与交互容器**，不改变业务状态和页面语义。

## 15.2 手机端

- 纵向叙事，一次只突出一个焦点；
- App Shell 固定底部 `窝 / 逛 / 遇见 / 图鉴`；
- `WHY`、出门纸条、筛选和短解释使用 Bottom Sheet；
- 重要 CTA 放在拇指容易触达区域，不把唯一 CTA 放到超长内容末尾；
- 不依赖 hover；
- Encounter 不硬塞左右两栏，而用上下/交替占屏表达双方；
- AWAY 等“空舞台”状态保留空白，不因为屏幕小而填满信息。
...
│ 144–176px  │ flexible / attention center  │ 0–320px      │
└────────────┴──────────────────────────────┴──────────────┘
```

- 左 Rail 承担品牌、四主导航和极少量全局状态；
- 中央舞台永远是当前页面第一视觉焦点；
- 右侧 Context **按需出现**，没有必要时直接让中央舞台扩展；
- Evidence、关系摘要、路线偏好适合 Drawer / Context，不与主舞台抢注意力；
- 桌面宽度变大不等于展示更多卡片；首屏注意力预算仍与手机相同；
- Reveal / Match / Encounter 可以利用横向空间形成舞台关系，但手机必须重新编排而不是压缩。
...
超过上限的信息进入折叠、Drawer/Sheet 或下一层页面。

## 15.5 视觉密度自检

如果用户在 3 秒内无法回答下面两个问题，页面必须继续减法：

1. “这一屏最重要的东西是什么？”
2. “我接下来最应该做什么？”

详细的 Desktop / Mobile 对照规范见：`docs/product/responsive-attention-spec-v1.md`。
