...
>
> 角色 IP 与资产生产已进一步冻结到 `docs/product/persona-art-system-v1.md`；外部图像生成/制作使用的逐阶段提示词见 `docs/product/art-generation-prompts-v1.txt`。生产顺序固定为：**视觉风格母版 → 六个角色母版 → 基于母版的动作/表情受控变体 → 世界与收藏物素材**。角色母版确认后禁止大范围重构。
