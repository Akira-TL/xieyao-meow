...
一趟由谢邀喵自主完成的有限知识漫游，是长期循环的服务端持久化事件单元；现有 ADR/PoC 中的 `Outing` 指同一领域概念。它不是现实地理旅行：猫从用户在知乎留下的真实兴趣、收藏、关注与创作等可用行为信号出发，在当时真实可用的知乎问题与知识内容中深挖、跨领域、找反方或偶遇陌生主题；用户最多提供弱路线倾向，最终内容、相遇与产物由 Persona、近期记忆、Journey 历史和知乎候选共同决定。主生命周期保持 `AT_HOME / PREPARING / AWAY / RETURNED`。

### Journey Event

Journey 处于 `AWAY` 时按预先计划发生的服务端持久化途中事件。P0 canonical 类型只有 `SCENE_POSTCARD`、`QUESTION_GLIMPSE`、`ENCOUNTER_GLIMPSE`；事件发生后即成为历史事实，刷新或晚打开不能重抽、重生成或改变结果。

### Event Plan

猫真正离家时为该 Journey 一次性确定的途中事件计划，只保存事件数量、类型与计划发生时间等结构化事实，不提前生成展示内容。到达计划时间后再 materialize；用户离线时也按原计划时间推进。

### 途中明信片

Journey Event 面向用户的可读投影，是旅途中的“预告”而不是回家奖励结算。用户是否打开、何时打开都不影响 Journey 路线或返回结果；正式产品不以弹窗或精确倒计时强迫领取。
...
Waiting Game v2 的唯一基础资源，是猫窝随时间自然积累并由 Journey 带回的旅行补给材料。它只用于补充单趟小物，不购买 Journey 次数、等待时间、关系或永久 Persona 能力；没有灵感叶时仍可空包旅行。
...
Explore 中由真实 Journey 逐步点亮的知识世界区域，按兴趣/知识领域组织而不是现实地理。只有猫真实到访过的地带才解锁；未探索地带只能以模糊剪影存在，用户不能把它直接指定为 Journey 目的地。

### 世界痕迹

猫在某个世界地带留下的轻量探索投影，例如代表场景、访问次数、最近旅途照、问题票根或遇见过的角色剪影。它用于说明“猫认识过哪里”，不是 Atlas / 收藏页的完整历史副本，也不承载实时知乎 Feed。
